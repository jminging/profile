...
function getCookie() {
	const key = $.KEY_UNIPAY_SIGN
	const session = {}
	session.url = $request.url
	session.body = $request.body
	session.headers = $request.headers

	if ( /newsign/.test(session.url) ) {
		const Cookie = $request.headers["Authorization"];
		if ( !!Cookie && $.setdata(JSON.stringify(session), key)) {
			$.subt = `获取会话: 成功! (${key})`
		} else {
			$.subt = `获取会话: 失败! (${key})`
		}
...
function sign() {
	return new Promise((resolve => {
		const url = JSON.parse($.getdata($.KEY_UNIPAY_SIGN))
